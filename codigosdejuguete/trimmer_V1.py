...
def main():
   ...
   #output trimeador
    ...
    #parametros=[ubicacion, mode, archivolog, entrada1, entrada2, salida1pareada, salida1nopareada, salida2pareada, salida2nopareada, adapt, seedm, pct, sct, leading, trailing, ws, req, minlen]
    
    illumina="ILLUMINACLIP:%s:%s:%s:%s"%(adapt,seedm, pct,sct)
    lead="LEADING:%s"%(leading)
    slidingwindow="SLIDINGWINDOW:%s:%s"%(ws,req)
    minl="MINLEN:%s"%(minlen)
    
    trimmomatic=["java","-jar",ubicacion,mode, "-trimlog", archivolog,entrada1,entrada2,salida1pareada,salida1nopareada,salida2pareada,salida2nopareada,illumina,lead,slidingwindow,minl]
    
    
    #trimmomatic=["java","-jar",ubicacion,mode, "-trimlog", archivolog,entrada1,entrada2,salida1pareada,salida1nopareada,salida2pareada,salida2nopareada,"ILLUMINACLIP:",adapt,":",seedm,":",pct,":",sct,"LEADING:",leading,"SLIDINGWINDOW:",ws,":",req,"MINLEN:",minlen]
    ...
    #Archivos  descriptores con permisos de escritura
    salidapantalla=open("salida.txt","w")
    salidaerror=open("error.txt",'w+')
    

    trimmer = subprocess.Popen(trimmomatic, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
   
    #sal=trimmer.stdout.read()
    #erro=trimmer.stderr.read()
    
    for linea in trimmer.stdout.read():
        sys.stdout.write(linea)
        salidapantalla.write(linea)
        
        
    for linea in trimmer.stderr.read():
        sys.stderr.write(linea)
        salidaerror.write(linea)
        
    ###captura de salida y posibles errores
    
    #cerramos los archivos para que se escriban los cambios y se liberen los buffers de I/O
  
    
    
    #captura= subprocess.Popen(trimmer, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell= True)
    #salidapantalla=open("salida.txt","w")
    #salidapantalla=trimmer.stdout
    
    #for linea in trimmer.stdout.readlines():
        #sys.stdout.write(linea)
        #salidapantalla.write(linea)
    #trimmer.wait()    
    ...
    ##Leemos lo que tengan los archivos y guardamos en las variables
    
    #while trimmer.poll() is None: #poll() retorna none mientras esta corriendo el subprocess
        #salidapantalla.write(1)
        
        #sys.salidapantalla.flush()
    
    #fd= open("salida.txt",'r')
    #output= fd.read()
    #fd.close()
    ...
    ##print(output)
        ...
        #print(output)
    #else:
        #print("se produjo el siguiente error: %s" %(err))
